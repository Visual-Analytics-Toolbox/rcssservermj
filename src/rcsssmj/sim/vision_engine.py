...
def v_compile(mj_spec: Any) -> VisionData:
    """Extract camera definitions and semantic markers, pre-allocating memory structures during compilation.
    ...
    owner_ids = np.array(owner_ids_list, dtype=str)
...
def v_recompile(mj_spec: Any, old_data: VisionData) -> VisionData:
    """Recompile sensor dictionaries and marker buffers when the simulator world undergoes structural changes.
    ...
    old_data: AudioData
        The previous vision state data.
    """

    new_data = v_compile(mj_spec)

    new_data.check_occlusion = old_data.check_occlusion
...
def v_step(
    ...
    for camera_site_name, cam in v_data.sensors.items():
        ...
        is_occluded_mask = np.full(n_markers, False, dtype=np.bool)

        # --- BATCH MUJOCO RAYCASTING ---
        if v_data.check_occlusion:
